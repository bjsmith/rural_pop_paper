...
preprocessed_data <- preprocessed_data_raw %>% filter((SID %in% unlist(sub_ids_to_remove))==FALSE) %>% filter(BMI>=25.0)
```
...
```{r}
predict_health_outcomes <- lm(BMI~PSS_sum+SRHI_healthy+SRHI_unhealthy+IPAQ_total_METminutes+Gender3C,
                  preprocessed_data)
